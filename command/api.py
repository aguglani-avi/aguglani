...
from Aguglani.cmd.exceptions import FastPathExistsError
from Aguglani.io import IVariableIOFormatter, VariableIO
from Aguglani.io.xml import VariableXmlStandardFormatter
from Aguglani.openmdao.problem import AutoUnitsDefaultGroup

# noinspection PyProtectedMember
from Aguglani.cmd.api import _get_simple_system_list

from Aguglani.utils.warnings import VariableDescriptionWarning

from . import resources

_LOGGER = logging.getLogger(__name__)

SAMPLE_FILENAME = "Aguglani.yml"
SAMPLE_XML_NAME = "Aguglani.xml"
BOOLEAN_OPTIONS = [
    "use_openvsp",
...
def generate_variables_description(subpackage_path: str, overwrite: bool = False):
    ...
    from Aguglani.command.api import generate_variables_description
    import my_package
    ...
    if not pth.exists(subpackage_path):
            ...
            for name in files:
                if name[-3:] == ".py":
                    ...
                    try:
                        ...
                        if system() != "Windows":
                            root_lib = ".".join(root.split("/")[root.split("/").index("Aguglani") :])
                        else:
                            root_lib = ".".join(
                                root.split("\\")[root.split("\\").index("Aguglani") :]
                            )
                            ...
                            try:
                                ...
                                if "propulsion_id" in my_class().options:
                                    ...
                                    for idx, _ in enumerate(available_id_list):
                                        ...
                                        if "Aguglani" not in available_id_list[idx]:
                                            idx_to_remove.append(idx)
        ...
        if pth.exists(pth.join(subpackage_path, "variable_descriptions.txt")):
            ...
            if dict_to_be_saved.keys():
                ...
                file.write("# Documentation of variables used in Aguglani models\n")
...
@deprecated(
    ...
    'Aguglani-core api with the distribution_name="Aguglani-cs23" instead',
)
...
def write_needed_inputs(
    problem: oad.AguglaniProblem, xml_file_path: str, source_formatter: IVariableIOFormatter = None
):
    """
...
def list_ivc_outputs_name(local_system: Union[ExplicitComponent, ImplicitComponent, Group]):
    ...
    for sub_system_keys in dict_sub_system.keys():
        if (
            ...
            and sub_system_keys.split(".")[-1] != "Aguglani_shaper"
        ):
...
def generate_block_analysis(
    ...
    OpenMDAO component (Implicit, Explicit or a Group), a registered Aguglani id, or the absolute
    ...
    if isinstance(local_system, str):
        if local_system.endswith(".yml"):
            configurator = oad.AguglaniProblemConfigurator(local_system)
    ...
    if not (os.path.exists(xml_file_path)) and not (set(var_inputs) == set(inputs_names)):
        ...
        if not (
            ...
            def patched_function(inputs_dict: dict) -> dict:
                ...
                problem_local = oad.AguglaniProblem()
...
def list_inputs_metadata(component: Union[om.ExplicitComponent, om.Group]) -> tuple:
    ...
    try:
        ...
        # noinspection PyProtectedMember
        vars_metadata = oad.AguglaniProblem()._get_undetermined_dynamic_vars_metadata(prob_copy)
        if vars_metadata:
