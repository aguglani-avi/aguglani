...
from Aguglani.models.performances.mission import resources
...
def test_oad_process_vlm_sr22(cleanup):
    ...
    logging.getLogger("Aguglani.module_management._bundle_loader").disabled = True
    logging.getLogger("Aguglani.openmdao.variables.variable").disabled = True

    # Define used files depending on options
    xml_file_name = "input_sr22.xml"
    process_file_name = "oad_process_sr22.yml"

    configurator = oad.AguglaniProblemConfigurator(pth.join(DATA_FOLDER_PATH, process_file_name))
...
def test_oad_process_mission_builder_1_engine(cleanup):
    ...
    source_mission_path = pth.join(pth.split(resources.__file__)[0], "sizing_mission_Aguglani.yml")
    target_mission_path = pth.join(WORKDIR_FOLDER_PATH, "sizing_mission_Aguglani.yml")
